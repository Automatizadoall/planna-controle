      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <CategoryIcon icon={budget.category?.icon ?? ''} className="text-2xl" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-semibold text-foreground">{budget.category?.name}</h3>
                {isOverBudget && (
                ...
                {isNearLimit && (
                  <span className="flex items-center gap-1 rounded-full bg-amber-100 dark:bg-amber-900/40 px-2 py-0.5 text-xs font-medium text-amber-600 dark:text-amber-400">
                    <TrendingUp className="h-3 w-3" />
                    Atenção
                  </span>
                )}
                {!isOverBudget && !isNearLimit && (
                  <span className="flex items-center gap-1 rounded-full bg-emerald-100 dark:bg-emerald-900/40 px-2 py-0.5 text-xs font-medium text-emerald-600 dark:text-emerald-400">
        ...
        <div className="mt-4">
          ...
          <div className="flex justify-between text-xs text-muted-foreground mt-1">
            ...
            <span>Alerta em {budget.alert_threshold}%</span>
          </div>
        </div>
      </CardContent>
    </Card>
